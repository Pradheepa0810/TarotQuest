    ...
    const ACHIEVEMENT_THEMES = {
      first_five: {
        className: 'ach-first-five',
        kicker: 'Seeker Ledger'
      },
      major_arcana_adept: {
        className: 'ach-major',
        kicker: 'Arcana Vault'
      ...
      swords_arcana_adept: {
        className: 'ach-swords',
        kicker: 'Air Court'
      },
      pentacles_arcana_adept: {
        className: 'ach-pentacles',
        kicker: 'Earth Court'
      },
      quiz_perfectionist: {
        className: 'ach-quiz',
        kicker: 'Precision Trial'
      },
      speed_learner: {
        className: 'ach-speed',
        kicker: 'Arcane Sprint'
      },
      tarot_master: {
        className: 'ach-tarot-master',
        kicker: 'Grand Arcana Seal'
      }
    };

    /* ─── Group definitions for milestones ─── */
    const GROUPS = [
      {
        id: 'major',
        label: 'Major Arcana',
        ...
        count: 22,
        color: 'major',
        completionTitle: 'Crown of the Arcana',
        award: 'Arcana Seeker Badge',
        trophyIcon: 'sparkles',
        flavor: 'You have walked the full Fool\'s Journey. Every archetype of the human soul now lives in your memory.',
        bonusGems: 350
      },
      {
        id: 'wands',
        ...
        count: 14,
        color: 'wands',
        completionTitle: 'Flameborne Adept',
        award: 'Flame Keeper Crest',
        trophyIcon: 'flame',
        flavor: 'You command the suit of Fire. Passion, vision, and creative force are yours to read.',
        bonusGems: 220
      },
      {
        id: 'cups',
        ...
        count: 14,
        color: 'cups',
        completionTitle: 'Tidemind Adept',
        award: 'Tide Walker Seal',
        trophyIcon: 'waves',
        flavor: 'You flow through the suit of Water. Emotion, intuition, and the heart\'s deepest currents speak to you.',
        bonusGems: 220
      },
      {
        id: 'swords',
        ...
        count: 14,
        color: 'swords',
        completionTitle: 'Skyblade Adept',
        award: 'Wind Blade Emblem',
        trophyIcon: 'wind',
        flavor: 'You have sharpened your mind on the suit of Air. Truth, clarity, and the power of thought are in your hand.',
        bonusGems: 220
      },
      {
        id: 'pentacles',
        ...
        count: 14,
        color: 'pents',
        completionTitle: 'Stoneheart Adept',
        award: 'Stone Root Medal',
        trophyIcon: 'leaf',
        flavor: 'You are grounded in the suit of Earth. Wealth, craft, and the wisdom of matter are your birthright now.',
        bonusGems: 220
      }
    ];

    /* ─── Hand-crafted lesson for The Fool ─── */
    const FOOL_CARD = {
      id: 'fool',
      name: 'The Fool',
      number: 0,
    ...
    let state = {
      ...
      completedGroups: [],
      rank: RANKS[0].title,
      masteredStatus: { deck: false, currentCard: false },
    ...
    function performCardSearch(query, resultsEl = null) {
      ...
      targetResults.innerHTML = results.map(card => `
        <button
          ...
          onclick="goToCardLesson('${card.id}')"
    ...
    function groupLearnedCount(groupId) {
      return state.learnedCards.filter(id => getCardGroup(id) === groupId).length;
    }

    function groupIsComplete(groupId) {
      const g = GROUPS.find(g => g.id === groupId);
      return groupLearnedCount(groupId) >= g.count;
    }

    function checkPendingMilestone() {
      for (const g of GROUPS) {
        if (groupIsComplete(g.id) && !state.shownMilestones.includes(g.id)) {
          return g.id;
        }
      }
      return null;
    ...
    function getCompletedGroups() {
      return GROUPS.filter(g => groupIsComplete(g.id)).map(g => g.id);
    }

    function isAchievementUnlocked(achievementId) {
      return state.achievementData.unlocked.includes(achievementId);
    }

    function unlockAchievement(achievementId) {
      const achievement = ACHIEVEMENTS.find(a => a.id === achievementId);
      if (!achievement || isAchievementUnlocked(achievementId)) return false;

      state.achievementData.unlocked.push(achievementId);
      ...
      enqueueCelebrationPopup({
        ...
        gemsLabel: achievement.gems > 0
          ? `+${achievement.gems} Gems · Total ${state.gems}`
          : `Collected in Alchemy Library · Total ${state.gems} Gems`
      });
    ...
    function getAchievementProgress(achievement) {
      switch (achievement.id) {
        case 'first_five':
          return {
            current: Math.min(state.learnedCards.length, achievement.target),
            detail: `${state.learnedCards.length}/${achievement.target} cards mastered`
          };

        case 'major_arcana_adept': {
          const majorCount = groupLearnedCount('major');
          return {
        ...
        case 'wands_arcana_adept': {
          const count = groupLearnedCount('wands');
          return {
        ...
        case 'cups_arcana_adept': {
          const count = groupLearnedCount('cups');
          return {
            current: Math.min(count, achievement.target),
        ...
        case 'swords_arcana_adept': {
          const count = groupLearnedCount('swords');
          return {
            current: Math.min(count, achievement.target),
        ...
        case 'pentacles_arcana_adept': {
          const count = groupLearnedCount('pentacles');
          return {
            current: Math.min(count, achievement.target),
        ...
        case 'quiz_perfectionist':
          return {
            current: Math.min(state.achievementData.perfectQuizWins, achievement.target),
            detail: `${state.achievementData.perfectQuizWins}/${achievement.target} perfect quizzes`
          };

        case 'speed_learner':
          return {
            current: Math.min(state.achievementData.bestSessionMastered, achievement.target),
            detail: `${state.achievementData.bestSessionMastered}/${achievement.target} cards in best session`
          };
    ...
    function evaluateAchievements() {
      if (state.learnedCards.length >= 5) {
        unlockAchievement('first_five');
      }

      if (groupLearnedCount('major') >= 22) {
        unlockAchievement('major_arcana_adept');
      }

      if (groupLearnedCount('wands') >= 14) unlockAchievement('wands_arcana_adept');
      if (groupLearnedCount('cups') >= 14) unlockAchievement('cups_arcana_adept');
      if (groupLearnedCount('swords') >= 14) unlockAchievement('swords_arcana_adept');
      if (groupLearnedCount('pentacles') >= 14) unlockAchievement('pentacles_arcana_adept');

      if (state.achievementData.perfectQuizWins >= 3) {
        unlockAchievement('quiz_perfectionist');
      }

      if (state.achievementData.masteredThisSession >= 3) {
        unlockAchievement('speed_learner');
      }

      if (state.learnedCards.length >= TarotDeck.TOTAL_CARDS) {
        unlockAchievement('tarot_master');
      }
    }

    function renderAlchemyLibrary() {
      const unlockedCount = state.achievementData.unlocked.length;
      const cardsHtml = ACHIEVEMENTS.map(achievement => {
        const unlocked = isAchievementUnlocked(achievement.id);
        const theme = ACHIEVEMENT_THEMES[achievement.id] || { className: '', kicker: 'Archive Card' };

        return `
          <article class="achievement-card ${theme.className} ${unlocked ? 'unlocked' : 'locked'}">
            <div class="achievement-back" aria-hidden="true">
              <div class="achievement-back-inner"></div>
            </div>
        ...
        <section class="card-panel alchemy-library-panel">
          <div class="alchemy-library-head">
            <div>
              ...
              <p class="alchemy-library-copy">Collected achievement cards: ${unlockedCount}/${ACHIEVEMENTS.length}</p>
            </div>
          </div>
    ...
    function renderHomePortal() {
      return `
        <section class="card-panel home-portal-panel">
          <div class="home-portal-hero">
            <h2 class="home-portal-title">Welcome, weary traveller</h2>
          ...
          <div class="home-action-grid">
            <article class="home-action-card">
              <div class="home-action-icon">${icon('sparkles')}</div>
              <h3 class="home-action-title">Start the Quest</h3>
              <p class="home-action-copy">Step beyond the cottage door, follow the whispering cards, and begin your tarot quest.</p>
              <button class="btn btn-primary" onclick="startQuestFromHome()">Start Lessons</button>
            </article>
          </div>
        </section>`;
    }
        ...
        <section class="card-panel chest-vault-panel">
          <div class="alchemy-library-head">
            <div>
              ...
              <p class="alchemy-library-copy">Your collected treasure and current standing.</p>
            </div>
          </div>
          <div class="chest-vault-image-wrap">
            <img class="chest-vault-image" src="Treasure_room.JPEG" alt="Treasure room" loading="lazy">
          </div>
          <div class="chest-vault-amount"><span class="icon-inline">${icon('gem')}<span>${state.gems} Gems</span></span></div>
          <p class="chest-vault-subcopy">Every lesson, quiz, streak, and achievement strengthens your chest.</p>
          <div class="chest-vault-grid">
            <div class="chest-vault-stat chest-vault-stat-coin">
              <div class="chest-vault-stat-label">Current Rank</div>
              <div class="chest-vault-stat-value">${getRank(state.gems)}</div>
            </div>
            <div class="chest-vault-stat chest-vault-stat-emerald">
              <div class="chest-vault-stat-label">Cards Mastered</div>
              <div class="chest-vault-stat-value">${progress.learned}/${progress.total}</div>
            </div>
            <div class="chest-vault-stat chest-vault-stat-ruby">
              <div class="chest-vault-stat-label">Review Due</div>
              <div class="chest-vault-stat-value">${due}</div>
            </div>
            <div class="chest-vault-stat chest-vault-stat-diamond">
              <div class="chest-vault-stat-label">Streak</div>
              <div class="chest-vault-stat-value">${state.daily.streak} days</div>
            </div>
          </div>
        </section>`;
    }
    ...
    function updateSidebar() {
      ...
      sbg.innerHTML = GROUPS.map(g => {
        ...
        return `
          <button type="button" class="sb-group ${isActive ? 'active-group' : ''} ${isDone ? 'group-complete' : ''}" onclick="jumpToGroup('${g.id}')">
      ...
      // Gems
      dom.sbGems.textContent = state.gems;
    ...
    function updateRightCardPanel(card, pos) {
      ...
      image.alt = card.name;
      image.classList.add('current-card-pulse');
      name.textContent = card.name;
      meta.textContent = `Card ${pos} of ${TarotDeck.TOTAL_CARDS} • ${card.suit} • ${card.element}`;
      keywords.innerHTML = card.keywords.slice(0, 4).map(k => `<span>${k}</span>`).join('');
    }
    ...
    function renderElemental(card) {
      ...
      const elementDisplay = relevantElement 
        ? `<div class="element-grid"><div class="element-chip"><span class="icon-inline">${icon(relevantElement.icon)}<span>${card.suit} = ${relevantElement.label}</span></span></div></div>`
        : '';
      return `
        <div class="card-panel">
      ...
      return `
        <div class="card-panel">
          ...
          <div style="text-align:center;margin-bottom:1rem">
            <img src="${card.image}" alt="${card.name}" style="width:120px;border-radius:6px;border:2px solid var(--gold-dim)">
          </div>
          ${items}
        </div>`;
    }
    ...
    function renderReversed(card) {
      ...
      return `
        <div class="card-panel">
          ...
          <div style="display:flex;flex-direction:column;align-items:center;gap:0.85rem;margin-bottom:1.2rem">
            <img class="reversed-card-img" src="${card.image}" alt="${card.name} Reversed" loading="lazy">
            <div style="font-size:0.78rem;color:var(--text-dim);text-align:center;font-style:italic">The card inverted — shadow emerges</div>
          </div>
        ...
        <div class="quiz-question" data-q="${qi}">
          ...
          <div class="quiz-options">
            ${q.options.map((opt, oi) => `
              <label class="quiz-option ${state.quizAnswers[qi] === oi ? 'selected' : ''}" onclick="selectAnswer(${qi}, ${oi})">
                <input type="radio" name="q${qi}" ${state.quizAnswers[qi] === oi ? 'checked' : ''}>
                <span>${opt}</span>
              </label>`).join('')}
          </div>
        </div>`).join('');

      const allAnswered = state.quizAnswers.length === card.quiz.length &&
        state.quizAnswers.every(a => a !== undefined);

      return `
        <div class="card-panel">
          <div class="phase-title">Oracle Quiz</div>
          <p style="margin-bottom:1.25rem;color:var(--text-dim)">Answer all questions — your score stays hidden until you submit. No peeking!</p>
          ${questions}
          <div class="nav-buttons">
            <button class="btn btn-primary" ${allAnswered ? '' : 'disabled'} onclick="submitQuiz()"><span class="icon-inline">${icon('sparkles')}<span>Face the Oracle.</span></span></button>
          </div>
        </div>`;
    }
    ...
    function renderMilestone(groupId) {
      ...
      const nextBtn = nextPendingReward
        ...
        : nextCard
           ? `<button class="btn btn-primary" onclick="startNextCard()">Move to Next Card.</button>
             <button class="btn btn-secondary" onclick="finishLesson()">Return Home</button>`
          : `<button class="btn btn-secondary" onclick="finishLesson()">Return Home</button>`;
    ...
    function renderResults(card) {
      const results = card.quiz.map((q, i) => {
        ...
        return { correct, q, chosen: state.quizAnswers[i] };
      });

      const score  = results.filter(r => r.correct).length;
      const pct    = Math.round((score / card.quiz.length) * 100);
      const earned = getCardGemReward(card);

      const grades = results.map((r, i) => `
        <div class="grade-item ${r.correct ? 'pass' : 'fail'}">
          <strong>Q${i+1}:</strong> ${r.correct
            ? `<span class="icon-inline">${icon('check')}<span>Correct!</span></span>`
            : `<span class="icon-inline">${icon('x')}<span>Not quite.</span></span>`}
          ${r.correct ? '' : `<br><small>You chose: "${r.q.options[r.chosen]}" — ${r.q.explain}</small>`}
        </div>`).join('');

      const nextId   = getNextCardId();
      const nextCard = nextId ? TarotDeck.CARDS[nextId] : null;
      const progress = TarotDeck.getDeckProgress(state.learnedCards);

      // Check if we just completed a group
      const pendingMs = checkPendingMilestone();

      if (!pendingMs && progress.learned >= progress.total) {
        return renderGrandmasterEnding(progress);
      }

      const nextBtn = pendingMs
        ? `<button class="btn btn-primary" onclick="goToMilestone('${pendingMs}')"><span class="icon-inline">${icon('award')}<span>Claim Your Reward</span></span></button>`
        : nextCard
          ? `<button class="btn btn-primary" onclick="startNextCard()">Move to Next Card.</button>`
          : `<div class="complete-banner" style="margin-bottom:1rem"><span class="icon-inline">${icon('crown')}<span>You mastered all ${TarotDeck.TOTAL_CARDS} cards!</span></span></div>`;

      return `
        <div class="card-panel results-panel">
          <div class="phase-title">Gem Reward</div>
          <p>You scored <strong>${score}/${card.quiz.length}</strong> (${pct}%)</p>
          <div class="gem-burst">+${earned} Gems</div>
          <div class="rank-badge">${getRank(state.gems)}</div>
          <p style="color:var(--text-dim);margin-bottom:1rem">Total Gems: ${state.gems} &nbsp;|&nbsp; Deck: ${progress.learned}/${progress.total}</p>
          ${grades}
          <div class="nav-buttons" style="margin-top:1.5rem">
            ${nextBtn}
            ${state.lastReviewedCard ? '<button class="btn btn-secondary" onclick="reviewLastCard()">Review Last Card</button>' : ''}
            <button class="btn btn-secondary" onclick="finishLesson()">Return Home</button>
          </div>
        </div>`;
    }
    ...
    function renderPhase() {
      /* ─── Compare mode takes over main content ─── */
      if (state.compareMode) {
      ...
      /* ─── SRS review session takes over main content ─── */
      ...
      const revealKey = `${state.currentCard}:${state.phaseIndex}`;
      if (phase.id === 'reveal') {
        if (state.revealViewKey !== revealKey) {
          state.revealCardFlipped = true;
          state.revealFlipAnimating = false;
          state.revealViewKey = revealKey;
        }
      } else {
        state.revealCardFlipped = false;
        state.revealFlipAnimating = false;
        state.revealViewKey = null;
      }

      if (phase.id === 'welcome' || phase.id === 'reveal') {
        setRightPanelCollapsed(true, 'system');
      } else {
        setRightPanelCollapsed(false, 'system');
      }

      const viewKey = `${state.currentCard}:${state.phaseIndex}`;
      if (state.lastViewKey && state.lastViewKey !== viewKey) {
        playElementTransition(card);
      }
      state.lastViewKey = viewKey;

      // Special: milestone screen overrides 'results' phase when pending
      if (phase.id === 'results' && state.quizSubmitted) {
        const pending = checkPendingMilestone();
        if (pending && state.showingMilestone) {
          dom.mainContent.innerHTML = renderMilestone(pending);
          updateHUD();
          refreshLucideIcons();
          return;
        }
    ...
    function nextPhase() {
      if (PHASES[state.phaseIndex]?.id === 'reveal' && (!state.revealCardFlipped || state.revealFlipAnimating)) {
        playUiSound('soft');
        return;
      }
      if (state.phaseIndex < PHASES.length - 1) {
        if (PHASES[state.phaseIndex]?.id === 'welcome') {
          state.lessonEntryCardId = null;
        }
        state.phaseIndex++;
        renderPhase();
        playUiSound('soft');
    ...
    function submitQuiz() {
      ...
      const score = card.quiz.reduce((sum, q, i) => sum + (state.quizAnswers[i] === q.answer ? 1 : 0), 0);
    ...
    async function resetProgress() {
      ...
      if (shouldReset) {
        state.gems            = 0;
        state.learnedCards    = [];
        state.currentCard     = 'fool';
        state.phaseIndex      = 0;
        state.quizAnswers     = [];
        state.quizSubmitted   = false;
        state.completedGroups = [];
        state.rank            = RANKS[0].title;
        state.masteredStatus  = { deck: false, currentCard: false };
        state.lastReviewedCard = null;
        state.navPosition      = 1;
        state.showingMilestone = false;
        state.popupQueue       = [];
        state.popupOpen        = false;
